...
public class ExcelToDatabase {
    ...
    public ExcelToDatabase(Activity activity) {
        this.activity = activity;
        this.dbAdapter = new DbAdapter2(activity);
    }
                ...
                if (sheet != null) {
                    ...
                    try {
                        for (int nRow = nRowStartIndex + 1; nRow <= nRowEndIndex; nRow++) {
                            ...
                            String TYPING_CONTENT = "";
                            String BUPMUN_CONFIRM = "0";

                            ScriptureDTO dto = new ScriptureDTO(IDX, BUPMUNINDEX, replaceChinese(TITLE1), replaceChinese(TITLE2), replaceChinese(TITLE3), replaceChinese(TITLE4), BUPMUNSORT,
                                                            CONTENTS.replace("·",".").replace("①", "1.").replace("②","2.").
                                                                    replace("③","3.").replace("④","4.").replace("⑤","5.").replace("⑥","6.").
                                                                    replace("⑦","7.").replace("⑧","8.").replace("⑨","9.").replace("⑩","10.").
                                                                    replace("⑪","11.").replace("⑫","12."), SHORTTITLE, PARAGRAPH_CNT, CHINESE_HELP, TYPING_CONTENT, BUPMUN_CONFIRM);

                            dbAdapter.createNote(dto);
